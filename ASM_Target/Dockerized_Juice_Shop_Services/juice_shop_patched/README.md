...
-p 127.0.0.1:3001:3000 keeps local port 3001 secure, opening it up only to the ASM_Target's loopback address. The service is exposed to the Internet through an Nginx reverse proxy server; The server acts as a VPN for a service similar to a VPN for a user.
